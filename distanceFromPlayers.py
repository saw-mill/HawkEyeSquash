...
while i < (len(frameList)-2):
    ...
    for cnt in contours:
        ...
        if M["m00"] != 0:
            ...
            continue
        cntsInfo.append((cnt, area, cX, cY))

    cntsInfo.sort(key=lambda tup: tup[1], reverse=True)
    if len(cntsInfo) >2 :
        if cntsInfo[0][1]-cntsInfo[1][1] > max_area:
    ...
    for c in candidates:
        ...
        cv2.putText(currFrame, str(minDist), (int(xmidPointPlayer), int(
            ymidPointPlayer)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
