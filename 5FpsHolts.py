    ...
    if(i % 5 == 0): # If first frame read 3 frames
        ...
        # If not the first frame
            ...
            if (len(ballCandidatesFilteredProximity) == 1):
                ...
                predXcoord.append(
                    round(levelEstimateXcoord[(i%5) -1] + trendEstimateXcoord[(i%5) -1], 2))
                predYcoord.append(
                    round(levelEstimateYcoord[(i%5) -1] + trendEstimateYcoord[(i%5) -1], 2))
                ...
                levelEstimateXcoord.append(alphaXcoord * ballCandidatesFilteredProximity[0][0] + (
                    1 - alphaXcoord) * (levelEstimateXcoord[(i%5) -1] + trendEstimateXcoord[(i%5) -1]))
                trendEstimateXcoord.append(
                    betaXcoord * (levelEstimateXcoord[i%5] - levelEstimateXcoord[(i%5) -1]) + (1 - betaXcoord) * trendEstimateXcoord[(i%5) -1])

                levelEstimateYcoord.append(alphaYcoord * ballCandidatesFilteredProximity[0][1] + (
                    1 - alphaYcoord) * (levelEstimateYcoord[(i%5) -1] + trendEstimateYcoord[(i%5) -1]))
                trendEstimateYcoord.append(
                    betaYcoord*(levelEstimateYcoord[i%5] - levelEstimateYcoord[(i%5) -1]) + (1 - betaYcoord)*trendEstimateYcoord[(i%5) -1])

                # Appending prediction for plotting
                dictFrameNumberscX[i + 1] = predXcoord[(i%5) -1]
                dictFrameNumberscY[i + 1] = predYcoord[(i%5) -1]

                cv2.circle(currFrame, (int(predXcoord[(i%5) -1]), int(
                    predYcoord[(i%5) -1])), 10, (0, 255, 0), -1)
                cv2.putText(currFrame, str(predXcoord[(i%5) -1]) + "," + str(predYcoord[(i%5) -1]), (int(
                    predXcoord[(i%5) -1]) + 1, int(predYcoord[(i%5) -1]) + 1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
            ...
            # If multiple candidates
                ...
                for cand in ballCandidatesFilteredProximity:
                    distncePredAct = math.sqrt(
                        math.pow((cand[0] - predXcoord[(i%5) -1]), 2) + math.pow((cand[1] - predYcoord[(i%5) -1]), 2))
                    # #drawing a line
                ...
                if (minDistObject == 1000):
                    cv2.circle(
                        currFrame, (int(predXcoord[(i%5) -1]), int(predXcoord[(i%5) -1])), 10, (0, 0, 255), -1)
                    x = predXcoord[-1]
                    y = predYcoord[-1]
                ...
                # If a candidate close to the prediction, use that for estimation
                    ...
                    cv2.circle(
                        currFrame, (int(predXcoord[(i%5) -1]), int(predYcoord[(i%5) -1])), 10, (0, 255, 0), -1)
            ...
            # If no candidate detected
                ...
                # print("m={}".format(m))
                predXcoord.append(
                    round(levelEstimateXcoord[(i%5) -1] + m*trendEstimateXcoord[(i%5) -1], 2))
                predYcoord.append(
                    round(levelEstimateYcoord[(i%5) -1] + m*trendEstimateYcoord[(i%5) -1], 2))
                ...
                cv2.circle(
                    currFrame, (int(predXcoord[(i%5) -1]), int(predYcoord[(i%5) -1])), 10, (0, 0, 255), -1)
                dictFrameNumberscX[i + 1] = predXcoord[(i%5) -1]
                dictFrameNumberscY[i + 1] = predYcoord[(i%5) -1]
                if(__debug__):
                    cv2.imshow('Candidate image', currFrame)
